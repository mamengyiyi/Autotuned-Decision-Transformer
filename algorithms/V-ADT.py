...
from iql import ImplicitQLearning, TwinQ, ValueFunction, DeterministicPolicy, GaussianPolicy
...
class IQLConfig:
    # Experiment
    device: str = "cuda:0"
    load_model: str = "/your/path/to/iqlmodel"
...
def load_iql_model(state_dim, action_dim, config, max_action=1):
    ...
    iql_q_func = trainer.qf.to(config.device)
    iql_actor = trainer.actor.to(config.device)
    return iql_value_func, iql_q_func
...
class TrainConfig:
    ...
    eval_seed: int = 42
    device: str = "cuda"
    load_iql_model: str = "/your/path/to/iqlmodel"  # Model load file name, "" doesn't load
...
class DecisionTransformer(nn.Module):
    ...
    def forward(
        ...
        sequence = (
            torch.stack([return_state_emb, act_emb], dim=1)
            .permute(0, 2, 1, 3)
...
def eval_rollout(
    ...
    states[:, 0] = init_state
    iql_value = iql_value_func(init_state.float()) * reward_scale
    returns[:, 0] = torch.as_tensor(iql_value, device=device)
    ...
    for step in range(model.episode_len):
        ...
        next_value = iql_value_func(torch.as_tensor(next_state, device=device).float()) * reward_scale
        # returns[:, step + 1] = torch.as_tensor(returns[:, step] - reward)
        returns[:, step + 1] = next_value.detach()
...
def train(config: TrainConfig):
    ...
    IQLConfig.device = config.device
    IQLConfig.load_model = config.load_iql_model
    IQLConfig.iql_deterministic = config.iql_deterministic
    IQLConfig.beta = config.iql_beta
    iql_value_func, iql_q_func = load_iql_model(config.state_dim, config.action_dim, IQLConfig)
    for step in trange(config.update_steps, desc="Training"):
        batch = next(trainloader_iter)
        states, actions, returns, time_steps, mask = [b.to(config.device) for b in batch]
        iql_q_values = iql_q_func(states.reshape(-1, config.state_dim), actions.reshape(-1, config.action_dim)).reshape(
            config.batch_size, config.seq_len) * config.reward_scale

        iql_v_values = iql_value_func(states) * config.reward_scale
        ...
        predicted_actions = model(
            states=states,
            actions=actions,
            returns_to_go=iql_q_values,
            time_steps=time_steps,
            padding_mask=padding_mask,
        )
        # loss = F.mse_loss(predicted_actions, actions.detach(), reduction="none")

        adv = iql_q_values - iql_v_values
        exp_adv = torch.exp(IQLConfig.beta * adv.detach()).clamp(max=EXP_ADV_MAX).unsqueeze(2)
        bc_losses = F.mse_loss(predicted_actions, actions.detach(), reduction="none")
